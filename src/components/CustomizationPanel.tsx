...
export const CustomizationPanel: React.FC<CustomizationPanelProps> = ({
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="space-y-4 p-4 rounded-xl bg-white/5 backdrop-blur-sm"
    >
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'effect' && (
          <motion.div key="effect" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
            <div>
              <label className="text-xs text-white/40 flex justify-between">
                <span>Độ dày</span>
            ...
            <div>
              ...
              <input type="range" min="0.5" max="2.5" step="0.1" value={sensitivity} onChange={(e) => setSensitivity(parseFloat(e.target.value))} className="w-full" />
            </div>
            <div className="flex flex-wrap gap-3 pt-1">
              <label className="flex items-center gap-2 text-xs text-white/60 cursor-pointer">
                <input type="checkbox" checked={glow} onChange={(e) => setGlow(e.target.checked)} className="w-4 h-4 rounded border-white/20 bg-white/5 text-accent-orange focus:ring-accent-orange" />
                <Sparkles className="w-3.5 h-3.5" /> Glow
              </label>
              <label className="flex items-center gap-2 text-xs text-white/60 cursor-pointer">
                <input type="checkbox" checked={particles} onChange={(e) => setParticles(e.target.checked)} className="w-4 h-4 rounded border-white/20 bg-white/5 text-accent-orange focus:ring-accent-orange" />
                <Sliders className="w-3.5 h-3.5" /> Hạt
              </label>
            </div>
            {glow && (
              <div>
                ...
                <input type="range" min="0.5" max="3" step="0.1" value={glowIntensity} onChange={(e) => setGlowIntensity(parseFloat(e.target.value))} className="w-full" />
              </div>
            )}
          </motion.div>
        )}
